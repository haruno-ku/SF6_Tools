...
// Any casing, or the fighter id. tools/lua/characters.lua already accepts all
// three, and two readers of the same bridge disagreeing about what counts as a
// name is its own defect - it made the command this repo's own README documents
// ("... zangief") exit 2 while the identical command in the Lua tools worked.
//
// Refusing an unknown name is still the point. Loosening the MATCH is not the
// same as falling back to a guess: an unrecognised name gets the known list and
// a non-zero exit, because guessing a filename is how a character silently ends
// up with no frame data.
const wanted = String(character)
const entry = BRIDGE.characters.find((c) =>
  c.catalog === wanted ||
  c.catalog.toLowerCase() === wanted.toLowerCase() ||
  String(c.fighter_id) === wanted)
if (!entry) {
...
// Canonical from here on, so the output filename does not inherit the caller's
// casing: `--character zANGIEF` must still write zangief.lua.
const catalogName = entry.catalog
...
const out = `data/frame-data/${catalogName.toLowerCase()}.lua`
...
body.push('--')
body.push('-- Street Fighter 6 frame data for ' + (data.name?.en ?? catalogName) + '.')
body.push('--')
