...
### Step 10.5 — Classic を測る 🆕

> **いま Classic のペアは1件残らず「押せない」として名指しで除外されている。**
> `SequenceCompiler` の `classic_buttons_unmeasured` が、LP/MP/HP/LK/MK/HK の
> どれかを押すステップを全部そこに入れる。`ryu-classic.json` の 486 ペアは
> **1回も試行されずに**除外されている。押せないからではなく、
> **どのビットがどのボタンか誰も測っていないから**で、
> 測らないまま押せば「繋がらない」という自信のある嘘が 486 行できる。
>
> `command_display` の `raw_button_mask` は答えにならない。あれは同じ技を
> **Modern で**押したときのマスクで、LP と LK が両方 16、MP と MK が両方 128、
> HP と HK が両方 256。Classic が必要としている区別が、そこには無い。
>
> **これを終わらせるのに必要なのはゲーム上の1回のランだけ。**

1. **ゲーム側でパッドを Classic にする。** トレーニングメニューの操作タイプ。
   ここが Modern のままだと、測れる6ビットは Modern のボタンのビットで、
   それが `classic_button_bits` に書き込まれる —— **もっともらしく、間違っている。**
2. **パネルの CALIBRATION で `[x] classic` を選ぶ。** 既定は `modern`。
   パネルはこの値をゲームから読まない（操作者が言う）。ただし
   トレーニングメニューの読みと食い違っていれば**赤で警告が出る**。
   両方のスキームについて「いま何ビット測れているか」がその場に並ぶ。
3. **パッドから手を離す。** Step 7-10 と同じ理由（書き込みは OR される）。
4. **START SWEEP。** あとは放置でよい。ボタンビットのフェーズは
   **12 ステップ**になる —— Classic には暫定値が1つも無いので、
   推測を検証するのではなく `InputMask.BTN_BITS` の**全ビットを1つずつ**押す。
   方向フェーズ（6ステップ）も同じ run の中で走る。
5. **何が観測できれば成功か**: 6ビット全部。LP/MP/HP/LK/MK/HK は
   31体すべてで単独表記（`LP` ×90, `LK` ×54, `HK` ×48, `HP` ×47, `MK` ×47,
   `MP` ×42）を持っているので、**押したビットが出した action_id を
   その単独表記のグループに照合するだけで名前が決まる。**
   4つしか出なければ `partial` で止まり、残り2つが `unwitnessed` に名前で残る。
   **`verified` にはならないし、出なかったビットに推測は入らない。**

**何が書かれるか**

| | |
|---|---|
| `calibration/latest-classic.json` | 起動時に読まれる Classic 側の設定。**`latest.json`（Modern）は上書きしない** |
| `calibration/<Char>-classic-<patch>-<stamp>.json` | 消えない記録。再走しても前回の名前は取らない |
| レジスタの `classic_button_bits` | `verified` / `partial` と、測れた6ビット |

Modern と Classic は**別々の測定で、別々のファイル**。3つの攻撃ボタンと6つの
攻撃ボタンは別物で、Modern のマップは Classic の4つについて何も言っていない。
起動時は両方読み、`generated_at` の**古い順**に適用する（新しい測定が勝つ）。

**そのあと何ができるようになるか**

```bash
# 開発機: Classic のワークリストを作る（すでに作れる。43a0148 から）
lua tools/lua/explore.lua Ryu --scheme classic --worklist
```

- `runtime/Sweep.lua` が `classic_button_bits` を読み、**測れたボタンを押すペアを
  除外しなくなる**。除外は「このスキームは押せない」から
  「**このステップは押せない**」に変わる —— 6ビット中5つしか測れていなければ、
  残る1つを押すペアだけが名前で除外される。
- `runtime/Injector.lua` は Classic のルートに対して
  `INJECTION_CLASSIC` のゲートを聞き、Classic のプロファイルを組む。
  Modern のマップが verified でも、それだけでは Classic のルートは通らない。

**このランのあとも Classic で測れないもの**（Modern と同じ理由で、同じ扱い）

- **溜め入力** `[4]6`。表記から角括弧が落ちるので "46" として1ティックずつ
  再生される。何ティック溜めればいいかは誰も測っていない（#34）。
  classic 表示で 116 件。
- **強度の無い表記** `P` / `PP` / `PPP` / `K` / `KK` / `KKK`。
  「どれかのパンチ」というボタンは存在しないので、`any_button` として
  コンパイル時に名前で拒否される。Modern の `任意键` と同じ扱い。
- **同方向の連打** `22`、および**空中始動**と**`>` 派生**。
  これも Modern と同じ理由で、Classic になっても変わらない。
- **THROW / DI**: 単独表記はあるが、ビットが取れるかは Modern 側と同じ問題
  （投げは中距離でスカり、カタログが unmapped と言う id が出る）。
  取れなければ `unwitnessed` に名前で残る。
...
**あとは放置。** 1試行およそ3秒。全キャラ分は `lua tools/lua/all.lua` が
ワークリストと一緒に書き出し、下の表もそのたびに書き直します（手で編集しないでください。
一覧ページは [`index.html`](index.html) / [`characters.md`](characters.md)）。
キャンセルのペアは隙間を数点試すので（`be1c0be`）、実際の所要時間は表より長くなります:

<!-- all.lua:pair-table -->
| キャラ | ペア数 | 1パス | DRC ペア（まだ押せない） |
|---|---:|---|---:|
| Ryu | 551 | 約28分 | 234 |
| Luke | 295 | 約15分 | 84 |
| Kimberly | 503 | 約25分 | 144 |
| ChunLi | 619 | 約31分 | 257 |
| Manon | 343 | 約17分 | 76 |
| Zangief | 159 | 約8分 | 61 |
| JP | 611 | 約31分 | 173 |
| Dhalsim | 664 | 約33分 | 158 |
| Cammy | 453 | 約23分 | 166 |
| Ken | 365 | 約18分 | 97 |
| DeeJay | 1572 | 約1.3時間 | 1862 |
| Lily | 532 | 約27分 | 204 |
| AKI | 330 | 約17分 | 30 |
| Rashid | 495 | 約25分 | 131 |
| Blanka | 532 | 約27分 | 105 |
...
**[`NEXT-SESSION.md`](NEXT-SESSION.md) を上から順に。** Ryu のキャリブレーション →
練習プランの掃引 → ルート実走（実測ダメージ）→ クラシックのボタンビット → DRC の順です。
この手順書（RUNBOOK）は初回ブリングアップの記録で、2回目以降は NEXT-SESSION が入口になります。
