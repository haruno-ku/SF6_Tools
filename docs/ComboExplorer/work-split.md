...
|---|---|
| カタログ行 | 3592 |
| 理論エッジ | **25551**（high 6298 / medium 4192 / low 15061） |
| ルート候補 | 5649（2手まで、survey のスコープ） |
| **情報不足による除外** | **0 件** / 12543 除外（31キャラ全部で） |
| 分類器が置けなかった行 | 52（監査開始時 187 → #21 で 76 → 派生の修正で 52） |
| 強さを名乗らない行 | 264（うち 24 は同一 motion の standalone 兄弟が無い） |
| 対象技の frame-data join | **1698 / 1719**（1554 → #23 で 1643 → 文脈付き lookup で 1698） |
| 始動技の frame-data join | 523 / 543 |
| 派生技の frame-data join | **184 / 286**（文脈付き lookup の前は 2） |
| 曖昧な frame-data join | 503（強さ展開が増えた分。絞れないものは match ではなく ambiguous） |
| 帰属が食い違う join | 187（#33。判定は変えず、黙らないようにしただけ） |
